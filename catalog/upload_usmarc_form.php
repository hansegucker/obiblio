...
<form enctype="multipart/form-data" action="../catalog/upload_usmarc.php" method="post">
    <?php echo $loc->getText("MarcUploadTest"); ?>: <?php echo $loc->getText("MarcUploadTestTrue"); ?><input
    ...
    <table border=0>
        ...
        <tr>
            ...
            <td><SELECT name=opac id=opac>
                    <option value=Y selected><?php echo $loc->getText("AnswerYes"); ?></option>
                    <option value=N><?php echo $loc->getText("AnswerNo"); ?></option>
                </select></td>
        </tr>
        <tr>
            <td colspan=2><input type=hidden name=userid id=userid value="<?php echo H($_SESSION["userid"]) ?>"></td>
        </tr>
    </table>
