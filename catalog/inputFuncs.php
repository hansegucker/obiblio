...
function printUsmarcInputText( $tag, $subfieldCd, $required, &$postVars, &$pageErrors, &$marcTags, &$marcSubflds, $showTagDesc, $cntrlType, $occur = "" ) {
	...
	echo '<label for="values[' . H( $formIndex ) . ']">';
	...
	if ( $cntrlType == OBIB_TEXTAREA_CNTRL ) {
		echo "<textarea class=\"materialize-textarea\" id=\"values[" . H( $formIndex ) . "]\" " .
		     "name=\"values[" . H( $formIndex ) . "]\" cols=\"" . H( $cols ) . "\" rows=\"" . H( $rows ) . "\">";
		echo H( $value ) . "</textarea>";
	} else {
		echo "<input type=\"text\"";
		echo " id=\"values[" . H( $formIndex ) . "]\" name=\"values[" . H( $formIndex ) . "]\" "
		     . "size=\"" . H( $size ) . "\" maxlength=\"" . H( $maxLen ) . "\" ";
		echo "value=\"" . H( $value ) . "\" >";
	}
	if ( $error != "" ) {
